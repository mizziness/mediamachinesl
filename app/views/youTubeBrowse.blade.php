	...
	<div id="youtube" class="media inner-container">
		...
		<div class="c2">
			<div id="side-nav">
				...
				<h3>Viewing:</h3>
				<p class="current">{{ $browseType }}:<br>{{ isset($prettyCats[$browseValue]) ? $prettyCats[$browseValue] : $browseValue }}</p>
				<h3>Browse By:</h3>
				<p>					
					...
					@if ( $results && isset($results["items"]) ) 
						...
						@foreach ( $results["items"] as $index => $result )
							...
							<div class="media-box cf">
								<a href="/youtube/play/{{ $result["id"]["videoId"] }}">
							...
							@if ( $index + 1 === $results["pageInfo"]["resultsPerPage"] )
								<div class="media-box cf">
									<a class="more" href="/youtube/browse/{{ $browseType }}/{{ $browseValue }}/{{ $nextPage }}">More Results &#187;</a>
								</div>
							@endif 							
							@if ( $count == 3 || $index + 1 === count($results["items"]) )
								</div>
							@endif
							<?php 
								$count++; 
